"""
------------------------------------------------------------------
- 객체 지향 프로그래밍 : topic3 견고한 객체 지향 프로그래밍 (SOLID)
- chapter1 : 단일 책임 원칙 (Single Responsibility Principle)

    - 하나의 클래스가 하나의 책임만 담당하게 구성한다.
    - “단일 책임 원칙은 같이 수정해야될 것들은 묶고, 따로 수정해야될 것들은 분리하는 것”
    - 프로그램의 유지 보수 차원에서 말하자면 한 클래스는 한가지 책임에 관한 변경사항이 생겼을 때만
      코드를 수정하게 되는 구조가 좋은 구조라는 뜻
"""
...
class Ship:
    ...
    def __init__(self, fuel, fuel_per_hour, supplies, num_crew):
        ...
        self.engine = ShipEngine(self.fuel_tank, fuel_per_hour)
